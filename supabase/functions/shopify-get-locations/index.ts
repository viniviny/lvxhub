...
serve(async (req) => {
  ...
  try {
    ...
    const res = await fetch(`https://${conn.store_domain}/admin/api/2026-01/locations.json`, {
      headers: { 'X-Shopify-Access-Token': conn.access_token, 'Content-Type': 'application/json' },
    });

    if (!res.ok) {
      const text = await res.text();
      return new Response(JSON.stringify({ error: 'Erro ao buscar localizações.' }), { status: res.status, headers: { ...corsHeaders, 'Content-Type': 'application/json' } });
    }

    const data = await res.json();
    const result = { locations: data.locations };
